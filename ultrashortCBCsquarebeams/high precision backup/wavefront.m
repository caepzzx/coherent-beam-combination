...
lambda=(lambda0-deltalambda/2*1.2):(2*deltalambda/50):(lambda0+deltalambda/2*1.2-2*deltalambda/50);
...
didx=8;%index difference for lambda
...
[pks2,locs2]=findpeaks(-I2(:,round(size(I2,2)/2)),x2);
...
Numofpoint=30;%Num of calcultaed point for a scale
scale=0.002;%scale factor for different wavefront
para_same(Numofpoint)=struct('SR',[],'SRdl',[],'ECdl',[],...
    'rof80',[],'PV',[],'RMS',[],'GRMS',[],'GPV',[],'I2',[]);
for scale=0.15;%scale factor for different wavefront
for m=1:Numofpoint
...
% [X2,Y2]=meshgrid(x2,y2);
    I2=abs(sum(u2,3).^2);
%     figure,imagesc(x2,y2,nthroot(I2,3));
    para_same(m).I2=I2(1:13:end,1:7:end);
    ...
    while ECenergySquare(I2,rof80x,rof80y,X2,Y2 )<0.8
            rof80x=rof80x+0.15*rdlx;
            rof80y=rof80y+0.15*rdly;
    end
...
save(['parsame_',num2str(scale),'.mat'],'para_same');%save result
clear u2 para_same;
end
...
% [X2,Y2]=meshgrid(x2,y2);
    I2=abs(sum(u2,3).^2);
%     figure,imagesc(x2,y2,nthroot(I2,3));
    para_diff(m).I2=I2(1:13:end,1:7:end);
...
save(['pardiff_',num2str(scale),'.mat'],'para_diff');%save result
clear u2 para_diff;
end

%% piston error for two beams 
Numofpoint=30;%Num of calcultaed point for a scale
piston=linspace(-1.5*pi,1.5*pi,Numofpoint);
...
u2(:,:,k)=u2t;
    ...
    I2=abs(sum(u2,3).^2);
%     figure,imagesc(x2,y2,nthroot(I2,3));
    para_piston(m).I2=I2(1:13:end,1:7:end);
...
save(['parpiston_','.mat'],'para_piston');%save 
clear u2 para_piston;
%% tip/tilt error for two beams 
Numofpoint=30;%Num of calcultaed point for a scale
tip=linspace(-3e-6,3e-6,Numofpoint);
...
u2(:,:,k)=u2t;
    ...
    I2=abs(sum(u2,3).^2);
%     figure,imagesc(x2,y2,nthroot(I2,3));
    para_tip(m).I2=I2(1:13:end,1:7:end);
...
save(['partip_','.mat'],'para_tip');%save 
clear u2 para_tip;
%% tip/tilt error for two beams 
Numofpoint=30;%Num of calcultaed point for a scale
tilt=tip;
...
u2(:,:,k)=u2t;
    ...
    I2=abs(sum(u2,3).^2);
%     figure,imagesc(x2,y2,nthroot(I2,3));
    para_tilt(m).I2=I2(1:13:end,1:7:end);
...
save(['partilt_','.mat'],'para_tilt');%save 
clear u2 para_tilt;
